...
$floris_single_thunbmail = ( sw_options( 'product_single_thumbnail' ) && !floris_mobile_check() ) ? sw_options( 'product_single_thumbnail' ) : 'bottom';
$post_thumbnail_id	 	  = get_post_thumbnail_id( $post->ID );
?>
<div id="product_img_<?php echo esc_attr( $post->ID ); ?>" class="woocommerce-product-gallery woocommerce-product-gallery--with-images images product-images loading" data-vertical="<?php echo ( ( $floris_single_thunbmail == 'left' || $floris_single_thunbmail == 'right' ) && !floris_mobile_check() ) ? 'true' : 'false'; ?>" data-video="<?php echo ( $floris_featured_video != '' ) ?  esc_url( 'https://www.youtube.com/embed/' . $floris_featured_video ) : ''; ?>">
	<figure class="woocommerce-product-gallery__wrapper">
	<div class="product-images-container clearfix <?php echo esc_attr( 'thumbnail-' . $floris_single_thunbmail ); ?>">
		<?php 
